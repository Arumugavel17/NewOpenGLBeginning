...
void Model::setup(int location, int vertex_attrib_size, bool normalized, int size, void* stride) {
    glVertexAttribPointer(location, vertex_attrib_size, GL_FLOAT, normalized, size * sizeof(float), stride);
...
void Model::process_node(const aiNode* node, const aiScene* scene) {
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++)
    {
        ...
        std::cout << "Loading Mesh Number" << i << "\n";
...
unsigned int Model::texture_from_file(const char* path, const std::string& directory) {
    ...
    if (data)
    {
        GLenum format = GL_RGB; // DEFAULT GL_RGB
        if (nrComponents == 1)
            format = GL_RED;
